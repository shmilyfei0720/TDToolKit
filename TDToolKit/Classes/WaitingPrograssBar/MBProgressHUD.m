...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    //view为空时 调用 initWithView 会主动抛出异常。所以这里加个判断 2018-07-26 czq
    if (!view) {
        NSLog(@"The view used in the MBProgressHUD initializer is nil");
        return nil;
    }
	MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view];
    NSString *path = [[NSBundle mainBundle] pathForResource:@"loadGif" ofType:@"gif"];
    ...
    hud.labelText = NSLocalizedString(@"device_alarm_load", @"加载中...");
	[view addSubview:hud];
	[hud show:animated];
#if __has_feature(objc_arc)
	return hud;
